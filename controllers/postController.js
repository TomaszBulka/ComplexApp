...
exports.edit = function(req, res){
    ...
    post.update().then((status) => {
        // the post was succesfully updated in the databasde
        // or the user did have permission but there were validation errors
        if(status == "success"){
            //post updated in the db
        ...
        // a post with a current id doesnt exists

        // or if the the current visitor is not the owner of the requested post
        req.flash("errors", "yo do not have permission to perform that action")
        req.session.save(function(){
            res.redirect("/")
        })
    })
...
exports.apiDelete = function(req, res){
    Post.delete(req.params.id, req.apiUser._id).then(() =>{
        ...
        res.json("You do nto have permission to perform that action")
    })
}
